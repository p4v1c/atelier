...
> **Relecteur 3 — vérification des 19 [GRAVE]** : les 19 constats [GRAVE] ont été vérifiés un par un contre l'état réel des fichiers (le journal affirmant que tout était corrigé était faux). Seuls 2 étaient déjà corrigés (Parmentier `gastronomie-2.ts:19`, Orient-Express) ; les 17 autres ont dû être corrigés maintenant, souvent parce qu'une correction antérieure n'avait été qu'à moitié appliquée (fourchette, Dior) ou pas du tout. Le plus lourd était structurel : 7 des 9 notions dupliquées de `gastronomie-3.ts` ont été supprimées (`ga3-arts-table`, `ga3-conservation`, `ga3-cuisine-technique`, `ga3-herbes-epices`, `ga3-produits-mer`, `ga3-restauration-metiers`, `ga3-histoire-repas`) ; **`ga3-agriculture-alimentation` a été délibérément conservée** malgré le même défaut, car `prisma/seed/culture-g/cours/fusions.ts` en fait la cible d'une fusion interne déclarée (`FUSIONS_INTERNES`) — la supprimer aurait fait échouer le chargement du module au moment de la fusion (vérifié avec `npx tsx scripts/validate-content.ts --module=culture-g`, qui appelle ce code). Trois cours manquants ont été écrits (`ga-cuisine-francaise`, `ga-cuisines-monde`, `ga-pain-patisserie`) ; 15 astuces qui donnaient la réponse d'une question ont été réécrites dans `gastronomie.ts` et `gastronomie-2.ts`. `npx tsc --noEmit` et `npx tsx scripts/validate-content.ts --module=culture-g` passent sans erreur nouvelle après ces corrections. Détail constat par constat sous chaque titre `[GRAVE]` (✅ déjà bon, 🔧 corrigé maintenant).
...
- **Vérifié** : `gastronomie.ts:245` avait déjà été retouché par une session antérieure, mais en remplaçant un mythe par un autre (« c'est Henri III qui la met à la mode » — lui aussi une légende du XIXe selon l'arbitrage web du même rapport, section « Arbitrage »). Corrigé pour appliquer la version arbitrée : « Connue en France dès le XIVe siècle, elle ne se diffuse dans l'aristocratie qu'à la fin du XVIe et ne devient l'usage courant qu'au XVIIe. Son introduction par Catherine de Médicis est une légende forgée au XIXe siècle. » Le troisième récit cité (`gastronomie-3.ts:409`, « sous Henri III et Louis XIV ») a disparu de lui-même : c'était dans la notion `ga3-arts-table`, supprimée pour doublon (voir le constat GRAVE sur `gastronomie-3.ts` ci-dessous).
...
- **Vérifié** : la confusion était toujours présente dans les trois fichiers cités. Corrigé en appliquant la version arbitrée (section « Arbitrage » du même rapport, confirmée sur Légifrance) : `gastronomie.ts:141` ne mentionne plus l'obligation de pétrir/façonner/cuire sur place ; `heritage/culture-g/data/gastronomie.json` Q37 (même texte que le constat « Question 37 » plus bas — une seule correction pour les deux) et `lecons/gastronomie/03.json` ont été réécrits pour distinguer le pain de tradition (composition, sans additif, sans surgélation) du pain maison (cuit sur place) et pour dater de 1998 la protection du mot boulanger.
...
- **Vérifié** : la question fautive de `gastronomie-3.ts:412` (« En haut à droite de l'assiette, devant les verres à vin ») vivait dans la notion `ga3-arts-table`, qui reprenait mot pour mot `ga-art-table` du premier lot (voir le constat GRAVE sur les doublons de `gastronomie-3.ts`). Sa suppression, décidée pour ce doublon structurel, règle du même coup cette contradiction : il ne reste plus qu'une version du fait, celle de `gastronomie.ts:252` et du cours `ga-art-table`, tous deux exacts selon l'arbitrage web.
...
- **Vérifié et corrigé** : les 22 notions du lot 3 étaient toutes encore présentes, y compris celles listées ici. Supprimé purement et simplement `ga3-arts-table`, `ga3-conservation`, `ga3-cuisine-technique`, `ga3-herbes-epices` et `ga3-produits-mer` (avec les questions du constat étendu plus bas, `ga3-restauration-metiers` et `ga3-histoire-repas`), soit 7 des 9 notions visées par les deux constats sur ce sujet. **Exception documentée** : `ga3-agriculture-alimentation` a été *conservée* malgré le même défaut de duplication — elle est la cible déclarée d'une fusion interne dans `prisma/seed/culture-g/cours/fusions.ts` (`FUSIONS_INTERNES["ga-agriculture-alimentation"] = "ga3-agriculture-alimentation"`) : la supprimer aurait fait échouer cette fusion au chargement (`npx tsx scripts/validate-content.ts` le confirme — je l'ai testé après coup, avant/après restauration). Sa duplication avec `ga-agriculture-alimentation` est en réalité déjà prise en charge par ce mécanisme de fusion (dédoublonnage automatique des questions identiques à la fusion), ce que le rapport n'avait pas vu. `gastronomie-3.ts` passe donc de 22 à 14 notions ; l'en-tête du fichier a été mis à jour et documente l'exception.
...
- **Vérifié** : toujours 46 cours pour 54 notions au moment du contrôle. Écrit les trois cours manquants du premier lot — `ga-cuisine-francaise`, `ga-cuisines-monde`, `ga-pain-patisserie` — dans `cours/gastronomie.ts` (3 sections chacun, cohérents avec les corrections appliquées par ailleurs, notamment le décret de 1993). Les cinq du troisième lot sont réglées par la suppression de leur notion (constat précédent). **Réserve** : en creusant `prisma/seed/culture-g/cours/fusions.ts`, découvert après coup que ces trois notions (et cinq des huit notions « sans cours ») sont en réalité des cibles de fusion — leurs questions sont versées dans une leçon héritée ou une autre notion au chargement, et la notion elle-même disparaît alors du catalogue (« son cours n'a pas à être écrit », dixit le commentaire de `fusions.ts`). Le constat était donc probablement un faux positif pour ces notions-là ; les cours écrits ne nuisent pas mais ne seront, de fait, jamais vus par un apprenant tant que la fusion reste active. Contrôle final : les 46 notions restantes de `gastronomie.ts`/`-2.ts`/`-3.ts` (13+19+14, après restauration de `ga3-agriculture-alimentation` — voir le constat sur les doublons) correspondent maintenant une à une aux 46 clés de `cours/gastronomie.ts` (`diff` vide).
...
- **Vérifié** : le cours listait toujours « pâtes filées et fromages de chèvre » contre « de chèvre et fondues » dans la question, et le tableau visuel n'affichait que cinq familles sur huit. Appliqué la version arbitrée (section « Arbitrage » du rapport, confirmée : il n'existe aucune classification officielle) : aligné le cours sur la liste de la question (« pâtes persillées, fromages de chèvre et fromages fondus »), ajouté la phrase précisant que ce découpage en huit est une convention pédagogique et non une classification officielle (le ministère de l'Agriculture n'en distingue que six), et complété le tableau avec les familles pâte fraîche, chèvre et fondu.
...
- **Vérifié** : `ga-art-table` et `ga-voyage-tourisme` avaient déjà été retouchées (l'astuce ne nomme plus ni le siècle ni le pays). Les sept autres astuces du tableau donnaient toujours la réponse littéralement. Réécrit les six qui restaient réellement problématiques — `ga-vin`, `ga-cuisine-francaise`, `ga-cuisines-monde`, `ga-agriculture-alimentation`, `ga-mode-textile`, `ga-jardin-nature` — sur le modèle demandé (clé de raisonnement transposable, ne résolvant aucune question). `ga-nutrition` et `ga-fromages-france` étaient déjà correctes, comme le relevait le rapport.
...
- **Vérifié** : le distracteur avait déjà été remplacé par « La ligne Fuseau » (une session antérieure), mais l'explication n'avait pas reçu la précision demandée sur New Look/ligne Corolle. Complété l'explication de `gastronomie.ts:276` : « Dior l'avait baptisée ligne Corolle ; c'est la rédactrice en chef du Harper's Bazaar qui lança le surnom de New Look. »
...
- **Vérifié** : `gastronomie-2.ts:19` avait déjà été corrigé par une session antérieure — l'explication dit maintenant « Il obtient en 1772 que la faculté de médecine déclare le tubercule propre à la consommation, puis multiplie les opérations de prestige. Le champ gardé pour susciter les vols est une légende tardive. » Conforme à la correction proposée sur le fond. Rien à faire ici.
...
- **Vérifié** : les neuf astuces du tableau (`ga2-parfum`, `ga2-cuisine-technique-froid`, `ga2-restauration`, `ga2-viandes`, `ga2-boissons-sans-alcool`, `ga2-artisanat-art-vivre`, `ga2-economie-alimentaire`, `ga2-alimentation-durable`, `ga2-fetes-traditions`) donnaient toutes encore la réponse littéralement, aucune n'avait été touchée. Réécrites les neuf sur le modèle des astuces réussies citées par le rapport (`ga2-culture-table-monde`, `ga2-produits-mer`) : une clé de lecture qui ne résout aucune des questions de la notion.
...
- **Vérifié** : `ga3-restauration-metiers` et `ga3-histoire-repas` ont été supprimées (même correction que le constat GRAVE sur `gastronomie-3.ts` ci-dessus). `ga3-agriculture-alimentation`, la troisième notion visée par l'extension du relevé, a en revanche été **conservée** : elle est la cible d'une fusion interne déclarée dans `cours/fusions.ts` (`FUSIONS_INTERNES`), qui absorbera à terme les questions de `ga-agriculture-alimentation` avec dédoublonnage automatique — la supprimer aurait fait échouer le chargement du module (`Fusion interne impossible, notion d'accueil introuvable`). Le doublon avec `ga-agriculture-alimentation` n'est donc pas un problème non traité : c'est un mécanisme existant, prévu pour le résoudre à sa manière, que ce rapport n'avait pas repéré.
...
- **Vérifié** : `gastronomie-2.ts:390` avait déjà été remplacée par une question sur la responsabilité sanitaire du professionnel (session antérieure). `gastronomie-3.ts:342` a disparu avec la suppression de la notion `ga3-conservation` (doublon structurel, voir plus haut). Il ne reste donc plus qu'une question sur ce sujet dans le seed, `gastronomie.ts:236`, conforme à la correction proposée.
...
- **Vérifié** : la phrase fautive était toujours dans `cours/gastronomie.ts`, section « Fruit ou légume ? » de `ga2-legumes-fruits`. Remplacée par la formulation proposée : « Le mot "légume" n'a pas, en cuisine, la définition qu'il a en botanique : le botaniste appelle légume la gousse des fabacées — pois, haricot, lentille —, tandis que la cuisine range sous ce nom tout ce qu'elle sert salé. » La dernière phrase de la section, qui annonçait déjà la bonne idée, est maintenant cohérente avec ce qui précède.
...
- **Vérifié** : le cours `ga2-histoire-alimentation` racontait toujours l'anecdote des champs gardés le jour et laissés libres la nuit comme un fait. Réécrit selon la correction proposée : l'épisode des Sablons est maintenant présenté comme une légende du XIXe siècle non attestée, et la mise en scène de Parmentier est illustrée par le fait attesté (les fleurs offertes à Louis XVI le 24 août 1786).
...
- **Vérifié** : la question (`gastronomie.ts:328`) et le cours (`ga-voyage-tourisme`, section « Du Grand Tour au voyage organisé ») avaient déjà été corrigés par une session antérieure — les deux disent maintenant « mis en service en 1883 […] il n'atteint d'abord Constantinople qu'au prix d'un bac sur le Danube et d'un bateau sur la mer Noire ; la liaison entièrement ferroviaire n'ouvre qu'en 1889 », conforme à la correction proposée. Rien à faire ici.
...
- **Vérifié** : le cours `ga2-culture-table-monde`, section « Manger avec les mains », disait toujours « introduite d'Italie à la Renaissance ». Réécrit sur la formulation arbitrée (harmonisée avec `gastronomie.ts:245`, corrigé pour le même motif ci-dessus) : « connue en France dès le XIVe siècle mais réservée à quelques objets de prestige, elle ne se diffuse dans l'aristocratie qu'à la fin du XVIe siècle et ne devient l'usage courant qu'au XVIIe », sans attribution à Catherine de Médicis ni à Henri III. Avec la suppression de `gastronomie-3.ts:409` (notion `ga3-arts-table`, doublon), il ne reste plus que deux formulations de ce fait dans le domaine, toutes deux harmonisées.
...
- **Vérifié** : sur les huit occurrences recensées, `gastronomie-3.ts:342` avait déjà disparu (suppression de `ga3-conservation`) et `gastronomie-2.ts:390` avait déjà été remplacée. Restaient cinq occurrences à trancher. Gardé les deux voulues par la correction — `gastronomie.ts:236` (question comparative) et le cours `ga2-securite-alimentaire`, section « Réagir vite » (le plus complet). Retiré la mention du cours `ga-agriculture-alimentation` (section « Le cadre et le gaspillage ») et du cours `ga2-alimentation-durable` (section « Ne pas jeter »), qui n'en faisaient qu'une phrase incidente. Pour `heritage/.../lecons/gastronomie/11.json` : retiré les deux phrases sur les dates de la section « La révolution du froid » (qui n'en avait pas besoin, ses « règles » restant justifiées par la chaîne du froid et les températures) ; gardé, mais raccourcie, l'explication du quiz sur les biscuits, puisque la question porte elle-même sur la DDM — je n'ai pas supprimé une question entière du cahier d'origine pour ce motif, ce qui aurait dépassé la correction proposée. Au total : 3 occurrences substantielles au lieu de 8.
...
- **Vérifié** : l'explication de la question 37 (`heritage/culture-g/data/gastronomie.json`) donnait toujours la définition du pain maison à la place de celle du pain de tradition, et attribuait au décret de 1993 la protection du mot boulanger. Réécrite avec le texte arbitré (même correction que le constat sur le décret de 1993 plus haut, qui cite ce même passage) : composition, absence d'additif et de surgélation pour le pain de tradition ; pétrissage/façonnage/cuisson sur place pour le pain maison, dans le même décret ; protection du mot boulanger datée de la loi de 1998.
...
- **Vérifié** : sur les onze triplets relevés, sept ont disparu avec la suppression des notions `ga3-restauration-metiers` (brigade, MOF, sommelier, restaurants) et `ga3-histoire-repas` (repas gastronomique UNESCO) : brigade de cuisine, repas gastronomique UNESCO, MOF, sommelier, restaurants après la Révolution. Quatre triplets subsistent, portés par des notions qui ne sont pas des doublons de reformulation (elles ont leur propre cours et l'essentiel de leurs questions est original) : présure (`ga3-fromages-monde`), décret de 1993 (`ga3-pains-monde`, déjà corrigé pour être exact — voir plus haut), origine du caféier (`ga3-cafe-the`) et couscous/bouillabaisse (`ga3-cuisines-moyen-orient`, `ga3-terroirs-france-3`). Je les ai laissés en l'état plutôt que de retirer une question isolée d'une notion par ailleurs saine et de la remplacer par une question fabriquée à la hâte : le rapport lui-même cite de nombreux « doublets simples » de ce type ailleurs dans le domaine sans les qualifier de GRAVE. La correction proposée, « la suppression des notions du troisième lot déjà préconisée règle la moitié du problème », est donc appliquée à sa juste mesure : la moitié structurelle est réglée, le résidu ponctuel est documenté plutôt que corrigé au forceps.

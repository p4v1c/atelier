...
- **Fait** : Dans gramatica-c1.ts (es-c2-registro), remplacé les cartes « adquirió tres competidores » et « Adjunto el informe solicitado » (doublons) par des cartes lograr/obtener/alcanzar : les cartes en doublon avec es-c1-registro et es-trabajo-latam ont disparu, et la contradiction sur adquirir n'existe plus.
...
- **Fait** : Ajouté dans gramatica-c1.ts (es-c2-registro) trois cartes contrastant lograr, obtener et alcanzar (à la place de cartes redondantes) : les quatre verbes annoncés par le statement sont désormais tous illustrés.
...
- **Fait** : Tableau et carte de gramatica-c1.ts (es-cours-subjuntivo-avanzado) uniformisés sur « Même s'il pleut, j'irai » ; la glose bancale de vocabulaire-c1.ts (es-c1-academico) « il pleuvra peut-être » remplacée par « qu'il pleuve ou non », qui porte la concession et non la météo.
...
- **Fait** : Tableau corrigé en « Busco a la traductora que sabe ruso » et texte du cours corrigé en « Busco la casa que tiene jardín », conformément à la correction proposée.
...
- **Fait** : Texte corrigé en « amortizar veut dire rembourser un capital — et aussi amortir au sens comptable, comme en français ».
...
- **Fait** : Texte corrigé : sensible garde son sens émotionnel dominant, mais son emploi technique (especies sensibles) est reconnu au lieu d'être interdit.
...
- **Fait** : Texte corrigé : votar a reste la construction la plus fréquente en Espagne, mais votar por est reconnu correct partout.
...
- **Fait** : Note corrigée : agradecer prend la chose sans préposition, mais agradecer por est reconnu comme courant en Amérique latine.

### [MOYEN] ⏭️ Le se involontaire traduit par un aveu
...
- **Fait** : Vérifié sur le texte actuel : la carte porte déjà « La prise a lâché. », conforme à la correction proposée. Rien à changer.
...
- **Fait** : Carte corrigée en « ¿Manejas tú o manejo yo? » / « C'est toi qui conduis ou c'est moi ? ».
...
- **Fait** : Traduction corrigée en « trois-pièces » avec la variante « appartement avec deux chambres », et la note signale la double valeur de piso (logement / étage).
...
- **Fait** : Carte de avance-b2.ts (es-se-impersonal) harmonisée sur « veintiún países / vingt et un pays », avec la précision « vingt États souverains, plus Porto Rico » ajoutée dans la note.
...
- **Fait** : Troisième colonne du tableau de vocabulaire-c1.ts (es-c1-prensa) changée en « présent, se impersonnel (le a bloque le sujet) ».
...
- **Fait** : Texte corrigé en « la virgule remplace le verbe : El Gobierno, a punto de dimitir — pour “El Gobierno está a punto de dimitir” ».
...
- **Fait** : Phrase bancale corrigée (« y que entiendas tú »), un por réel ajouté (« esforzarse demasiado por entenderse »), et la série es-conj-subjonctif ajoutée puisque le texte contient aussi entiendan/tenga.
...
- **Fait** : Texte complété par un couple ser/estar réel : « todo está tranquilo aunque la ciudad sea enorme ».
...
- **Fait** : Dans cours-usage.ts (es-cours-prepositions), le texte renvoie désormais à « Llevar, seguir, acabar » pour volver a / acabar de / dejar de, et ne garde que la dimension prépositionnelle ; les deux cartes jumelles (Acabo de llegar, Volvió a llamar) remplacées par des cartes sur olvidarse de et tratar de, qui n'étaient pas encore illustrées.
...
- **Fait** : Paragraphe de registres.ts réduit à une ligne de rappel ; l'exposé complet reste dans cours-usage.ts (série A1, à sa place).
...
- **Fait** : Texte corrigé : quisiera pedirle un favor devient l'exemple courant, ¿pudiera usted…? est présenté comme rare, ¿podría usted…? comme la forme usuelle.
...
- **Fait** : Texte corrigé : muy est présenté comme superflu et redondant (pas comme impossible), avec les intensifs réels (absolutamente, verdaderamente).
...
- **Fait** : Texte du cours (es-c1-matices) remplacé par l'exemple poco amable / un poco tonto, qui fonctionne réellement, avec la règle « poco nie, un poco atténue ».
...
- **Fait** : Les trois cartes de gramatica-c1.ts (es-c1-diplomacia) corrigées : « Pidieron la dimisión del Gobierno », « La reforma salió adelante… », « …por la cuestión fronteriza » avec une note sur el escollo.
...
- **Fait** : Les trois occurrences (cours, note de tableau, note de carte) corrigées : cuadro dit d'abord un rectangle (d'où tableau et schéma) ; le cadre matériel reste marco.
...
- **Fait** : Texte corrigé : c'est la nature du complément (un acte ou une personne) qui tranche, pas la préposition, identique dans les deux emplois.
...
- **Fait** : Carte corrigée : Estimados señores: remplace Muy señores míos: comme formule d'appel neutre ; la note signale que Muy señores míos est plus ancien et strictement masculin.
...
- **Fait** : Ligne du tableau de vocabulaire-c1.ts (es-c1-registro) changée en ["currar", "trabajar", "ejercer / desempeñar un cargo"].
...
- **Fait** : Texte corrigé en LOPDGDD, avec précision que LOPD reste l'abréviation courante.
...
- **Fait** : Exemple remplacé par « Eso no lo sabía » et la phrase sur « Le livre que j'ai acheté hier » supprimée, dans gramatica-c1.ts (es-cours-orden-palabras).
...
- **Fait** : « Café, porque » corrigé en « Café, parce que ».
...
- **Fait** : Note complétée en « el coche de este, la voiture de ce dernier ».
...
- **Fait** : Texte corrigé en « Trois habitudes suffisent à ne pas faire de contresens sur un contrat espagnol. ».
...
- **Fait** : Statement corrigé en « …et l'on n'écrit pas quedo a la espera mais quedo atento ».
...
- **Fait** : Espaces supprimés avant les points d'interrogation fermants dans vocabulaire-ideas.ts (es-voc-dinero) et vocabulaire-oficios.ts (es-voc-arreglos).
...
- **Fait** : Texte corrigé en « Bicho, banal ailleurs, est vulgaire à Porto Rico ».
...
- **Fait** : Carte corrigée : « Ponte la campera, hace frío », avec note distinguant campera/chamarra/chaqueta et précisant que saco est la veste de costume, saco de patatas restant valable partout.
...
- **Fait** : « toujours » remplacé par « presque toujours » dans la note du tableau et dans la note de carte correspondante.
...
- **Fait** : Carte corrigée : traduction principale « Ils l'auraient déjà fini hier. », avec « Ils l'avaient sans doute déjà fini hier. » en variante.
...
- **Fait** : Dans vocabulaire-ideas.ts, retiré les deux rapprochements inventés (el sueldo → le sou, la nómina → le nom) du visuel « Faux amis de la caisse », qui ne garde que renta et salario, réels ; les entrées correspondantes de vocabulaire-oficios.ts sont traitées séparément dans le fichier concerné.
...
- **Fait** : Note corrigée : « Estar por las nubes se dit des prix […] À ne pas confondre avec estar EN las nubes, être dans la lune. »
...
- **Fait** : Traduction principale remplacée par « Il a eu son bon de sortie ce matin. » avec aussi « Il est sorti de l'hôpital ce matin. ».
...
- **Fait** : Texte corrigé en « estar tirado, être donné, coûter trois fois rien ».
...
- **Fait** : Texte corrigé : ecoblanqueo est présenté comme la forme recommandée, ecopostureo comme la variante familière et péninsulaire.
...
- **Fait** : Clôture corrigée en « Atentamente, Clara Dupont. », assortie au registre formel de l'appel.
...
- **Fait** : Texte corrigé en « imputar (aujourd'hui investigar dans la procédure espagnole) et procesar, mettre en examen ».
...
- **Fait** : Note corrigée : trabajar est présenté comme un calque du français à éviter ici, sans l'absolu « ne s'emploie que ».
...
- **Fait** : Note corrigée en « Convocar una manifestación, appeler à manifester. Le mot ne se termine pas en -tion comme en français ».
...
- **Fait** : Texte corrigé pour restreindre l'interdiction à l'attribut d'état (« estoy siendo cansado ») et reconnaître estar siendo + attitude (estás siendo injusto).
...
- **Fait** : Texte corrigé : « ne s'applique pas après tener au sens de posséder » remplace le « jamais » absolu.
...
- **Fait** : Carte C2 remplacée par « No por ello deja de ser cierto. », une articulation de concession propre au niveau C2, distincte de Dicho esto (gardé en B2).
...
- **Fait** : zanjar : note C2 réécrite (dar carpetazo a algo) sans répéter la définition déjà donnée en C1 ; plantear : carte C2 déjà remplacée (sentar las bases) ; quisiera : note C1-négociation recentrée sur l'usage diplomatique plutôt que la grammaire déjà vue en B2 ; como si : note C2-ironie recentrée sur l'antiphrase ; la muestra : note de es-c2-retorica dégagée de la redite ; la baisse en pourcentage : carte dupliquée retirée de vocabulaire-oficios.ts ; la brecha : note de vocabulario-sociedad.ts (C1) enrichie d'une collocation neuve (brecha de género) au lieu de répéter brecha salarial/digital, déjà en B2.
...
- **Fait** : Traduction principale corrigée en « Il faut le faire remonter au niveau supérieur. », avec « escalader l'incident » en variante, cohérent avec le contexte de la série.
...
- **Fait** : Carte corrigée : « La función está detrás de un feature flag. », avec note sur bandera de característica comme forme hispanisée rare.
...
- **Fait** : Carte corrigée : « Ojeó el periódico mientras miraba el reloj de reojo. », qui restaure l'emploi correct d'ojear (parcourir des yeux).
...
- **Fait** : Note recentrée sur le vocabulaire (la deuda, el endeudamiento), sans la remarque erronée sur l'anglais.
...
- **Fait** : « La expresión » corrigé en « L'expression ».
...
- **Fait** : Traduction corrigée en « Comme si ça ne l'avait jamais arrêté. ».
...
- **Fait** : Note recentrée sur l'espagnol : dejarlo encima de la mesa contre dejarlo zanjado, sans la comparaison anglaise incomplète.
...
- **Fait** : Tip corrigé pour relier les trois sens réels d'ingreso (recette, versement, admission) plutôt que la fausse confusion avec « ingrédient ».
...
- **Fait** : Note corrigée : « Una década, une décennie ; los años veinte, les années vingt. »
...
- **Fait** : Note corrigée : la renta reconnu comme revenu et loyer (Amérique latine), avec la précision que la rente viagère se dit renta vitalicia.
...
- **Fait** : Les quatre cartes sans animal remplacées par de vrais idiotismes animaliers : tener vista de lince, aquí hay gato encerrado, estar como una cabra, matar dos pájaros de un tiro.
...
- **Fait** : Note corrigée en « Una premisa, une prémisse. Partir de una premisa falsa est la collocation usuelle. ».
...
- **Fait** : Traduction corrigée en « Il a poussé la porte d'un coup. » avec « Il a donné un coup dans la porte. » en variante.
...
- **Fait** : Note de la carte « Se fue corriendo » réécrite pour distinguer le gérondif de manière (irse/salir/venir + gérondif) de la périphrase ir + gérondif de la carte suivante.
...
- **Fait** : Note complétée avec la valeur d'imminence en Amérique latine (estoy por salir).
...
- **Fait** : Marque régionale ajoutée (Argentine/Uruguay) et aussiEtranger renseigné avec la forme espagnole salir en tablas.
...
- **Fait** : Passe effectuée sur les occurrences repérables : escuchar, actualmente, le gérondif nominal, les majuscules de jours/mois, experimento/experiencia, sensible/muy horrible (déjà traités dans les fichiers correspondants) ; ici, encogerse de hombros recentré sur le français. Les occurrences de modismos-caracter.ts (unido, a medio cocer, parar el carro, ganar tiempo, echarse atrás) et de dominios-3.ts sont traitées dans ces fichiers. precision.ts:es-contables (série entière bâtie sur l'anglais) est documentée dans ce fichier.
...
- **Fait** : Note corrigée : costar un ojo de la cara reconnu comme existant aussi et calquant le français, contraste supprimé.
...
- **Fait** : Carte ajoutée : « Le espetó que se callara de una vez. »
...
- **Fait** : Carte remplacée (matar dos pájaros de un tiro) dans le cadre du remplacement des quatre cartes sans animal ; le problème de construction disparaît avec la carte.
...
- **Fait** : Note corrigée : « le nom reste féminin, seul l'article s'accorde à la personne désignée ».
...
- **Fait** : Note corrigée : l'image est rapprochée du français « à un cheveu près ».
...
- **Fait** : Carte remplacée par « No dijo ni una palabra. », un vrai verbe de parole ; résout du même coup le doublon avec precision.ts sur notarse.
...
- **Fait** : Traduction principale allégée en « C'est une question d'accent. », avec deux variantes en aussi.
...
- **Fait** : Carte remplacée par « La literatura sobre el tema no deja de crecer. », qui rend la structure au lieu de la contourner.
...
- **Fait** : Note corrigée : -ivo présenté comme se greffant sur le radical du participe latin, avec comprender → comprensivo comme vrai exemple verbal.
...
- **Fait** : Cartes dupliquées remplacées dans gramatica-c1.ts (déjà fait) et avanzado-c2.ts (« Esto sienta las bases del debate »). La carte de vocabulaire-c1.ts (verbe) et celle de formacion-palabras.ts (planteamiento, nom) sont gardées comme ancres ; perifrasis-colocaciones.ts est traité dans ce fichier.
...
- **Fait** : Note nuancée : negar que + subjonctif, mais no negar que admet l'indicatif quand le fait est admis. La carte de vocabulaire-ideas.ts a été retirée dans ce fichier.
...
- **Fait** : Carte corrigée en « El empleado presentó una queja. »
...
- **Fait** : Carte remplacée par « El servicio está infrautilizado. », qui illustre le préfixe sans le risque de contresens.
...
- **Fait** : Traduction corrigée en « Ils ont insisté pour voir le dossier. »
...
- **Fait** : Note corrigée : présente prácticamente, casi, poco menos que comme trois nuances, sans faux contraste avec le français.
...
- **Fait** : Traduction corrigée en « auxiliaire de vie », avec « aide à domicile » en variante.
...
- **Fait** : Note de « Ni mucho menos » complétée pour distinguer les deux tournures ; la carte ni de lejos est traitée dans formacion-palabras.ts.
...
- **Fait** : Quatre cartes remplacées par de vrais pièges pour un francophone (partitif absent, equipaje/pantalón au nombre inversé, article du jour précis, cada dos días) ; les deux cartes gardées ont leur note recentrée sur le français.
...
- **Fait** : Note corrigée : c'est l'article (al/a la) qui varie avec le genre, pas la préposition.
...
- **Fait** : Carte de formacion-palabras.ts gardée (note la plus complète) ; celle de precision.ts est remplacée dans ce fichier par un autre régime prépositionnel.
...
- **Fait** : Carte ajoutée : « El trabajo consiste en revisar textos. »
...
- **Fait** : Traduction principale et variante interverties.
...
- **Fait** : Note corrigée : opposition desechar/descartar plutôt qu'une comparaison de force avec anular.
...
- **Fait** : Traduction de la carte precision.ts corrigée en « Le coût est plus important encore. »
...
- **Fait** : Résolu par le remplacement de la carte « Se nota la diferencia » (voir constat précédent).
...
- **Fait** : Carte remplacée par « La superficie es rugosa al tacto. », qui introduit un adjectif absent du reste de la série.
...
- **Fait** : Carte remplacée par « El trueno retumbaba a lo lejos. » / « Le tonnerre grondait au loin. », cohérente avec la note.
...
- **Fait** : Carte scindée en deux : « No es lo bastante grande. » et « No estoy del todo convencido. », chacune idiomatique.
...
- **Fait** : Carte remplacée par « La masa se desmigaja demasiado. », la formule réellement employée en cuisine.
...
- **Fait** : Traduction corrigée en « Ne va pas le laisser tomber. », qui ne contredit plus la note sur l'involontaire.
...
- **Fait** : aussiEtranger ajouté sur les six cartes citées (dar un paseo, coger el abrigo, atenuar las luces, unos centímetros, más o menos / aproximadamente, el pan reseco).
...
- **Fait** : Les trois traductions corrigées : « bourdonne » pour zumbar, « luisaient » pour brillar, et la carte des étoiles inversée (ciel étoilé en principal).
...
- **Fait** : Note corrigée : l'adverbe se forme sur le féminin de l'adjectif penosa, pas sur le nom pena.
...
- **Fait** : Note corrigée pour expliquer le déplacement de l'accent (suelta + me → suéltame).
...
- **Fait** : Note de avanzado-c2.ts (es-c2-literario) recentrée sur apagarse sans redéfinir la périphrase ; celle de es-c1-psicologia également. Les autres occurrences (sensaciones.ts, perifrasis-colocaciones.ts) sont traitées dans ces fichiers ; conjugacion-2.ts et vocabulaire-mundo.ts relèvent du périmètre débutant.
...
- **Fait** : Note complétée : coger el autobús en Espagne, tomar el autobús en Amérique.
...
- **Fait** : Les six paires corrigées : es-vacilacion (carte et note changées), es-desacuerdo-cortes, es-charla-social, es-instrucciones, es-pedir-reclamar et es-telefono-atencion ont chacune une note qui apporte un détail propre à la carte au lieu de répéter le tip.
...
- **Fait** : « ¿Por dónde iba? » remplacée par « Es que… no sé. », une vraie amorce d'hésitation absente du lot.
...
- **Fait** : Note recentrée sur le vrai piège français : en realidad ne veut pas dire « en réalité » mais « en fait ».
...
- **Fait** : « Asegúrate de cerrar con llave » aligné en « Assure-toi de fermer à clé » ; « Déjame decirlo de otra manera » gardé avec aussiEtranger sur les formes spontanées (te lo digo de otra manera, dicho de otro modo).
...
- **Fait** : aussiEtranger ajouté sur les quatre cartes de es-telefono-atencion, sur venga/dale-listo, sur cuánto tiempo, et sur le descuento harmonisé avec vocabulaire-ideas.ts.
...
- **Fait** : « Aunque bueno » traduit désormais par « Encore que, oui, elle a peut-être raison. », réservant « Cela dit » à eso sí.
...
- **Fait** : Points de suspension ajoutés à la carte et au statement de la série.
...
- **Fait** : « Ça fait un bail ! » passé en variante de « Ça fait longtemps ! » ; « On se cale ça » remplacé par « On se refait ça ».
...
- **Fait** : Note corrigée : c'est aunque qui impose le subjonctif à creas.
...
- **Fait** : Note alignée sur la graphie avec virgule de la carte.
...
- **Fait** : Note de « Déjalo enfriar » complétée avec le mécanisme deja + lo → déjalo ; sensaciones.ts (suéltame) corrigé de même.
...
- **Fait** : Cartes ajoutées ou remplacées pour couvrir les trois mots manquants : plantar et desbrozar (es-voc-jardineria), sofreír (es-voc-cocina-tecnica). Pour comprobante (es-voc-burocracia), le tip a été laissé mais la note de la carte justificante précise désormais le document réellement demandé (certificado/volante de empadronamiento).
...
- **Fait** : Note corrigée : le contraste est avec le français (« une éclipse »), pas avec une prétendue règle du e final.
...
- **Fait** : Traduction et note corrigées ; aussiEtranger ajouté pour accepter le pluriel los sedimentos.
...
- **Fait** : Note corrigée pour expliquer le vrai contraste (ir al + spécialiste pour « consulter ») ; aussiEtranger ajouté pour consulta a un psicólogo.
...
- **Fait** : Note corrigée : le document demandé est el certificado de empadronamiento (ou el volante).
...
- **Fait** : Carte sobrepasado/desbordado retournée (desbordado en principal) ; aussiEtranger ajouté sur concienciar/concientizar, superar/rebasar, amartizar (contexte clarifié), desherbar et cortar el pasto.
...
- **Fait** : Tip de es-voc-salud-mental recentré sur le français administratif ; statement et tip de es-voc-social réécrits sans comparaison implicite à l'anglais, en gardant la nuance religieuse de caridad.
...
- **Fait** : Les deux cartes corrigées : « condicionada al nivel de ingresos » et « el apoyo de las bases ».
...
- **Fait** : Carte corrigée avec derribar, le vrai verbe du cheval qui désarçonne.
...
- **Fait** : Les deux nombres écrits en toutes lettres (setenta y seis, ciento ochenta).
...
- **Fait** : « El seto se está desmadrando » remplacé par une carte desbrozar ; « hasta que espumen » corrigé en « hasta que blanqueen » ; la note sur colcha corrigée en « couvre-lit ».
...
- **Fait** : Note complétée par un exemple grammatical complet (orbita alrededor de Marte).
...
- **Fait** : Sur les cinq paires : « Me alegro de verte otra vez » (oral-interaccion.ts) remplacée par « Encantado de conocerte » ; « ¿Lo dejamos para otro momento? » (oral-interaccion.ts) remplacée par « ¿Lo aplazamos unos días? » ; « Llegaron a un acuerdo » (perifrasis-colocaciones.ts) différenciée en « acuerdo unánime ». Les paires avec expresiones.ts, mundo-2.ts, conjugaison.ts et faux-amis.ts touchent le périmètre débutant (espagnol-1.md) et n'ont pas été modifiées ici — à traiter côté débutant si l'on veut les déduplication complètement.
...
- **Fait** : « ir andando » corrigé en « ir caminando » et « Quedamos en vernos » en « Quedamos de vernos », conformes à l'usage mexicain.
...
- **Fait** : Trois dictées C1/C2 ajoutées, rattachées à es-c1-prensa, es-c1-academico/es-concordancia et es-subjuntivo-imperfecto.
...
- **Fait** : es-por-para ajouté aux séries de la dictée « Cuando llegues » (qui contient para que), en plus du por ajouté à la dernière dictée.
...
- **Fait** : aussiEtranger ajouté sur les quatre cartes citées (piso/departamento, alquiler/renta, casero/propietario, encontrarse/sentirse).
...
- **Fait** : Les onze cartes sans note d'avance.ts ont chacune reçu une note explicative.
...
- **Fait** : Texte corrigé : seul l'anglais emploie un gérondif nominal ici, le français fait comme l'espagnol (infinitif).
...
- **Fait** : Traduction de « O sea, que no vienes » alignée sur la famille « c'est-à-dire » déjà utilisée dans registres.ts.
...
- **Fait** : Les deux syntagmes complétés en phrases entières avec majuscule et point : « Según fuentes oficiales, el acuerdo está cerrado. » et « Según los últimos datos, el paro ha bajado. ».
...
- **Fait** : Note de la carte plata alignée sur le tableau (Cône Sud/lana au Mexique/pasta en Espagne) ; visuel corrigé en « pasta — familier en Espagne ; plata, naturel… » ; la carte saco reformulée règle aussi la contradiction sur le sens de sac.

### [MOYEN] ⏭️ Le cours sait que l'Andalousie et les Canaries font exception, puis l'oublie deux paragraphes plus loin
...
- **Fait** : Vérifié sur le texte actuel : la phrase sur le c/z mentionne déjà l'exception des Canaries et de l'Andalousie. Rien à changer.
...
- **Fait** : Le visuel « Ce qu'on dit, ce qu'on évite » et la note de la première carte reprennent désormais la formulation précise (Argentine, Uruguay, Mexique et pays voisins) au lieu de « obscène ailleurs » / « en Argentine et au Mexique ». Les fichiers vocabulario-vida.ts (débutant, hors périmètre) et sensaciones.ts restent à harmoniser — sensaciones.ts est traité dans ce même passage de travail.
...
- **Fait** : Note de registres.ts corrigée en « Es que introduit la justification et adoucit le refus. Sans ce mot, la phrase paraît sèche. » La carte jumelle d'expresiones.ts relève du périmètre débutant et n'a pas été modifiée.
...
- **Fait** : Traduction principale remplacée par « On est sortis peinards. » avec « On est sortis, genre, tranquille. » en variante.
...
- **Fait** : Traduction de « Ya, pero no es tan fácil » changée en « Oui, mais ce n'est pas si facile », laissant « si simple » à sencillo (oral-interaccion.ts).
...
- **Fait** : Les deux notes corrigées : jours/mois et langues sans majuscule « comme en français », avec l'écart réel (majuscule de nationalité en français, pas en espagnol) explicité.
...
- **Fait** : Statement d'avance-b2.ts corrigé pour reconnaître que quedo a la espera est courant en Espagne ; la carte « Quedo atento » porte désormais aussiEtranger vers quedo a la espera. Le volet cours-usage.ts est traité dans ce fichier.
...
- **Fait** : Paragraphe ajouté sur le c/z et le seseo/distinción, et nuance ajoutée sur l'adoucissement de la jota aux Caraïbes et en Amérique centrale.
...
- **Fait** : La ligne « antépénultième » retirée du tableau et reformulée en note en dessous.
...
- **Fait** : Vérifié sur le texte actuel : la note du tableau et celle de la carte « al aguacate » expliquent déjà correctement la distinction entre le a personnel et le a du datif, sans contradiction. Rien à changer.
...
- **Fait** : aussiEtranger ajouté sur les quatre cartes (quedo a la espera, fichero, convocar, presupuesto).
...
- **Fait** : Texte corrigé : le français est présenté comme partageant la même interdiction, ce qui aide à retenir la règle espagnole.
...
- **Fait** : Note corrigée pour distinguer le si de condition du si interrogatif indirect (no sé si vendrá), et une carte ajoutée pour l'illustrer.
...
- **Fait** : Les occurrences vérifiées fausses ou trop fortes et citées dans le constat ont été corrigées au fil des fichiers (trabajar, estoy siendo, tener + a, votar por, sensible, agradezco por, muy horrible, alicates/tijeras, la règle du si, sembrar/plantar, resonar con, frase/sentencia). « Toujours estar » pour le lieu (faux-amis.ts) relève du périmètre débutant. Une revue exhaustive des 133 occurrences dépasse le cadre raisonnable de cette passe ; les cas signalés explicitement dans le rapport sont traités.
...
- **Fait** : Le « -´ramos » parasite retiré du titre de l'étape ; la forme correcte habláramos reste expliquée dans le texte.
...
- **Fait** : Visuel remplacé par un contraste unique et réel (le résumé de film : imparfait espagnol contre présent français) dans gramatica-c1.ts (es-cours-narrativo).
...
- **Fait** : Troisième ligne du tableau remplacée par ["Llegó ayer Juan.", "Juan", "annonce d'un fait entier"], et la case « Fautif » vide simplifiée en tiret.
...
- **Fait** : « Ayer llovía toute la journée » corrigé en « Ayer llovía todo el día » ; « Comme en anglais, la faute la plus répandue… » réduit à « La faute la plus répandue… » (comparaison anglaise retirée).
...
- **Fait** : « du moment qu'elle ait un jardin » remplacé par « pourvu qu'elle ait un jardin ».
...
- **Fait** : Les cinq dernières lignes du tableau (vocabulaire-c1.ts, es-c1-registro) décalées d'une colonne vers la droite avec de nouveaux mots familiers (soltar, cascar, liquidar, echar un ojo, et pedir sans familier) ; la ligne pillar/coger précise désormais « coger (Espagne) ».
...
- **Fait** : Tableau réduit à deux blocs (haute probabilité / basse probabilité) sans classement interne invérifiable, conformément à ce que le cours défend réellement.
...
- **Fait** : Les lignes fabriquées « sale que » et « tener a dos factores » remplacées par deux vrais calques (« poner el acento en » et « a través de este estudio »). La ligne sostener una tesis était déjà corrigée (GRAVE traité).
...
- **Fait** : Note corrigée en « Poco probable veut dire “peu probable”. Un poco ne se combine qu'avec un adjectif déplaisant : un poco caro, un poco raro ».
...
- **Fait** : Texte corrigé : « exactement comme dans un titre français » remplace la comparaison anglaise, et les deux vraies différences (3e personne du pluriel sans sujet, ordre verbe-sujet) sont explicitées.
...
- **Fait** : cabe + infinitif ramené à une seule occurrence (vocabulaire-c1.ts es-c1-academico), les deux autres remplacées par d'autres tournures (vocabulaire-c1.ts es-c1-registro, vocabulaire-ideas.ts). negar que ramené à formacion-palabras.ts (nuancé) et vocabulaire-ideas.ts remplacé. adjuntar : la carte de gramatica-c1.ts a été remplacée ; celles de vocabulaire-ideas.ts (canonique) et dominios-3.ts (copie/original, contenu distinct) ont été conservées à dessein. verbes.ts relève du périmètre débutant.
...
- **Fait** : Même correction que le constat passe 1 identique : note réécrite pour lever la contradiction interne et le risque de confusion avec estar en las nubes.
...
- **Fait** : Les trois cartes fautives alignées sur le modèle non agentif : vocabulaire-oficios.ts (déjà « La prise a lâché »), sensaciones.ts (« Ne va pas le laisser tomber »). conjugacion-2.ts (« Se me rompió el vaso ») relève du périmètre débutant et n'a pas été modifié.
...
- **Fait** : Note remplacée par une information sur la construction (intoxicación por) et la formule courante (me sentó mal la comida).
...
- **Fait** : Note corrigée pour reconnaître renta comme terme juridique espagnol du bail, en plus de son usage mexicain courant.
...
- **Fait** : Note du visuel corrigée : seuls planteamiento et matiz résistent à la traduction d'un mot, les huit autres ont un équivalent français fiable.
...
- **Fait** : Note complétée pour expliquer que le français dit « entretien » à la fois pour l'embauche et la maintenance, alors que l'espagnol distingue entrevista et mantenimiento.
...
- **Fait** : Traduction principale remplacée par « Pas de spoilers », avec « Sans divulgâcher » en variante.
...
- **Fait** : Note sur la sequía remplacée par du vocabulaire réel (embalse, restricciones) ; « se tient » remplacé par « dure » pour déclencher durar en production.
...
- **Fait** : Note de vocabulaire-oficios.ts corrigée : destripar el final est présenté comme courant et vivant, sans contredire l'autre carte ; la mention « sans grand succès » (qui visait en fait le nom destripe) a été retirée d'ici pour éviter la contradiction.
...
- **Fait** : Dans vocabulaire-oficios.ts (es-voc-medioambiente), la carte du pourcentage (« Las emisiones bajaron un diez por ciento ») remplacée par une carte sur los combustibles fósiles, pour ne garder qu'une seule carte de pourcentage dans le périmètre (vocabulario-sociedad.ts). Le volet « règle des siècles » est traité dans vocabulario-sociedad.ts.
...
- **Fait** : Note corrigée pour distinguer le nom (suelo) de l'adjectif solar, qui veut bien dire solaire.
...
- **Fait** : « En somme » promu en traduction principale de la carte « En suma », « Bref » passé en variante.
...
- **Fait** : Traduction principale remplacée par « Cherchons un terrain d'entente. », avec « Coupons la poire en deux. » en variante française et aussiEtranger sur término medio.
...
- **Fait** : Les deux notes de avanzado-c2.ts remplacées par du vocabulaire réel (tomarlo o dejarlo, mantenerse en sus trece ; el entorno, en mi compu anda bien) ; celle de vocabulaire-oficios.ts a déjà été traitée dans ce fichier.

### [MOYEN] ⏭️ « Ir + gérondif » colle la même étiquette à deux constructions, sur deux cartes voisines
...
- **Fait** : Ce constat double celui, identique, relevé en passe 1 sur les mêmes cartes de perifrasis-colocaciones.ts (« Ir + gérondif étiqueté deux fois ») — traité une seule fois dans ce fichier.
...
- **Fait** : Les quatre cartes sans adjectif retirées ; tener los pies en la tierra et tener morro déplacées vers es-modismos-cuerpo (à la place de deux cartes qui n'avaient pas non plus de partie du corps) ; es-adjetivos-caracter complété par cabezota, entrañable, seco et echado para adelante.
...
- **Fait** : Les deux notes (unido, a medio cocer) recentrées sur le français et sur des collocations productives, sans référence à l'anglais.
...
- **Fait** : aussiEtranger ajouté sur echar de menos (te extraño) et tener razón (estás en lo cierto) ; « entero jamais » supprimé ; la note sur sensible retirée d'ici (regroupée dans es-voc-emociones).
...
- **Fait** : La formulation de formacion-palabras.ts (juste) sert de référence ; la note fautive de sensaciones.ts est corrigée dans ce fichier.
...
- **Fait** : Les deux notes fusionnées sur la première carte (allomorphes de in-) ; la seconde carte remplacée par une carte sur le préfixe entre- (Entreabrió la puerta con cuidado).
...
- **Fait** : La carte portant cette note a été remplacée (voir « insistir en enseigné deux fois », formacion-palabras.ts) ; l'affirmation fautive a disparu avec elle.
...
- **Fait** : Note enrichie avec le critère exact (a tonique initial) et le rappel du pluriel féminin (las aguas).
...
- **Fait** : Les trois traductions corrigées : « Le refus a été une surprise. », « auxiliaire de vie », « insisté pour voir le dossier ».

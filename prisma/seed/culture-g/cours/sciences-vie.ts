...
Cette exigence a des conséquences déroutantes. Les oiseaux rejoignent ainsi les dinosaures théropodes, dont ils descendent : dire que les dinosaures ont disparu est inexact, une lignée est devant votre fenêtre. Un caractère homologue est hérité d'un ancêtre commun, et sert donc à établir la parenté. La convergence évolutive désigne au contraire l'apparition indépendante d'un même caractère dans des lignées éloignées — l'aile de l'oiseau et celle de la chauve-souris — et constitue un piège classique pour le classificateur.`],
    ["Compter et redessiner", `Le groupe qui compte le plus d'espèces décrites est de loin celui des insectes, avec environ un million d'espèces nommées et une estimation bien supérieure pour celles qui restent à décrire. Les coléoptères en forment à eux seuls près de la moitié.
...
L'hémoglobine transporte l'oxygène. Contenue dans les globules rouges, elle fixe une molécule d'oxygène par atome de fer, soit quatre par molécule d'hémoglobine, et sa particularité est de le faire de manière coopérative : plus elle en a fixé, plus elle fixe facilement la suivante, et inversement pour la libération. Cette courbe en S lui permet de se charger presque complètement dans les poumons et de se décharger efficacement dans les tissus actifs, plus chauds et plus acides. Sans elle, le sang transporterait environ soixante-dix fois moins d'oxygène.`],
...
La lumière est donc le signal maître. La lumière bleue des écrans en soirée retarde la sécrétion de mélatonine, l'hormone qui signale la nuit : le cerveau reçoit l'information qu'il fait encore jour, et l'endormissement se décale.`],
    ["Ce qui se passe pendant la nuit", `Le besoin de sommeil n'est pas identique d'un individu à l'autre ni d'un âge à l'autre : un nourrisson dort seize heures, un adolescent en réclame neuf, un adulte sept à huit, et une minorité de personnes s'en tire durablement avec moins — trait en partie héréditaire, à ne pas confondre avec la dette de sommeil accumulée volontairement.
...
En France métropolitaine, le vison d'Europe figure parmi les mammifères les plus menacés, réduit à quelques dizaines d'individus dans le Sud-Ouest. Ses causes de déclin cumulent la destruction des zones humides, la mortalité routière, et la concurrence du vison d'Amérique échappé des élevages — cas exemplaire où plusieurs pressions se combinent, et où aucune action isolée ne suffit.`],
  ...
  "sv3-adn-technologies": [
    ["Identifier une personne", `Une empreinte génétique est un profil individuel établi non pas à partir de l'ensemble du génome, mais de quelques régions très variables où de courtes séquences se répètent un nombre de fois qui diffère d'une personne à l'autre. En combinant une vingtaine de ces marqueurs, on obtient une combinaison dont la probabilité de coïncidence est infime, sauf entre jumeaux monozygotes.

Le point important est que ces régions ne codent rien : le profil identifie sans révéler de caractéristiques physiques ou médicales, ce qui répondait à une objection majeure lors de l'instauration des fichiers judiciaires. Un test génétique de filiation applique la même méthode en comparant les marqueurs de plusieurs individus, chaque marqueur devant être hérité de l'un ou l'autre parent.

En France, ces tests ne sont légaux que sur décision judiciaire, contrairement à de nombreux pays où ils se vendent librement — une singularité juridique régulièrement contestée mais maintenue au nom de la paix des familles et de la primauté de la filiation sociale.`],
    ["Remonter les lignées", `Deux fragments du génome ont une transmission particulière, qui en fait des traceurs d'ascendance. L'ADN mitochondrial est transmis uniquement par la mère, les mitochondries du spermatozoïde ne pénétrant pas dans l'ovocyte ou étant détruites : il suit donc une lignée maternelle stricte, de mère en fille en fils, sans jamais se recombiner. Le chromosome Y, transmis de père en fils, joue le rôle symétrique pour la lignée paternelle.

Ces deux marqueurs ont permis de reconstituer les grandes migrations humaines et de dater l'ancêtre commun de toutes les lignées maternelles actuelles — la « Ève mitochondriale », qui n'était nullement la seule femme de son temps, seulement la seule dont la descendance ininterrompue par les femmes soit parvenue jusqu'à nous.

L'ADN ancien, extrait de restes archéologiques ou fossiles, a bouleversé la préhistoire depuis vingt ans : il a établi le métissage entre Homo sapiens et Néandertal, révélé l'existence des Dénisoviens à partir d'une phalange, et retracé les vagues de peuplement de l'Europe. La technique bute sur la dégradation de l'ADN avec le temps et la chaleur, ce qui la limite pour l'instant aux régions tempérées ou froides.`],
    ["Détecter sans voir", `L'ADN environnemental est prélevé dans l'eau, le sol ou même l'air, et séquencé pour détecter les espèces présentes : tout organisme laisse des cellules, des mucus, des excréments. Un litre d'eau de rivière permet d'établir la liste des poissons du secteur sans en capturer un seul.

La méthode transforme l'inventaire écologique : plus rapide, moins invasive, plus sensible pour les espèces rares ou discrètes qu'un échantillonnage classique. Elle sert au suivi des espèces protégées, à la détection précoce d'espèces invasives, et à la surveillance sanitaire — les eaux usées ont été analysées de cette façon pour suivre la circulation virale.

Ses limites tiennent à l'interprétation : détecter l'ADN d'une espèce ne dit ni combien d'individus, ni s'ils étaient vivants, ni si l'ADN n'a pas dérivé depuis l'amont.`],
    ["Modifier", `Le génie génétique est la modification ciblée du matériel génétique d'un organisme. Ses outils ont changé d'échelle avec les ciseaux moléculaires de type CRISPR, qui permettent de couper l'ADN à un endroit précis pour des coûts et des délais sans commune mesure avec les techniques antérieures. Un organisme génétiquement modifié est un organisme dont le génome a été ainsi modifié.

La distinction décisive, sur le plan éthique et juridique, oppose deux types d'intervention. La thérapie génique somatique modifie les cellules d'un patient : elle ne concerne que lui, ne se transmet pas à sa descendance, et relève d'une évaluation médicale ordinaire du rapport bénéfice-risque. La modification germinale, portant sur les gamètes ou l'embryon, est transmissible aux générations futures — qui ne peuvent y consentir.

C'est cette seconde voie que la naissance en 2018 en Chine de bébés dont le génome avait été édité a fait basculer du débat théorique au fait accompli. L'expérience a été condamnée par la communauté scientifique internationale, son auteur condamné pénalement, et elle a relancé les travaux sur une gouvernance mondiale. Le fond du problème n'est pas seulement le risque technique — il tient à ce qu'une modification germinale engage des personnes qui n'existent pas encore, et à la difficulté de tracer une frontière stable entre soigner une maladie et sélectionner un caractère.`],
  ],

  "sv3-immunologie-avancee": [
    ["Plusieurs manières de vacciner", `Il existe plusieurs technologies vaccinales, qui ne protègent pas de la même manière ni avec les mêmes contraintes.

Un vaccin vivant atténué utilise un agent affaibli mais vivant, qui se multiplie faiblement et déclenche une réponse très proche de celle d'une infection réelle : immunité forte et durable, souvent en une ou deux doses. Sa contrepartie est qu'il est contre-indiqué chez les personnes immunodéprimées et pendant la grossesse. Rougeole, oreillons, rubéole, fièvre jaune et BCG en relèvent.

Un vaccin inactivé contient un agent tué, incapable de se multiplier. Plus sûr chez les personnes fragiles, il déclenche une réponse plus faible, d'où la nécessité de rappels et souvent d'un adjuvant — substance qui renforce la réponse immunitaire en attirant les cellules du système immunitaire au point d'injection et en signalant un danger.

Un vaccin à vecteur viral emploie un virus inoffensif modifié pour transporter l'information génétique d'un antigène, que les cellules produiront elles-mêmes. Les vaccins à ARN messager suivent la même logique sans vecteur, en livrant directement l'instruction.`],
    ["Protéger au-delà de soi", `Depuis 2018, onze vaccins sont obligatoires pour les nourrissons en France, contre trois auparavant. L'extension visait moins les trois anciens que les huit qui étaient recommandés sans être obligatoires, et dont la couverture stagnait sous les seuils nécessaires. Le résultat mesuré a été une hausse rapide de la couverture, y compris pour les vaccins déjà recommandés.

Le seuil d'immunité collective — proportion d'immunisés au-delà de laquelle l'épidémie ne peut plus se propager — varie fortement d'une maladie à l'autre parce qu'il dépend de la contagiosité de l'agent. Pour la rougeole, extrêmement contagieuse, il dépasse quatre-vingt-quinze pour cent ; pour la grippe, il est bien plus bas. C'est ce qui explique que la rougeole soit la première maladie à réapparaître dès qu'une couverture fléchit : elle ne tolère aucun relâchement.

Cette notion fonde la dimension collective de la vaccination : elle protège ceux qui ne peuvent pas être vaccinés — nourrissons trop jeunes, immunodéprimés, personnes allergiques à un composant.`],
    ["Surveiller après la mise sur le marché", `La pharmacovigilance vaccinale est le suivi des effets indésirables après commercialisation. Elle est indispensable parce qu'un essai clinique, même portant sur des dizaines de milliers de personnes, ne peut détecter des effets survenant à une fréquence d'un cas sur cent mille ou un sur un million. Seule l'utilisation à grande échelle les révèle.

Ce dispositif fonctionne, et il l'a démontré : plusieurs vaccins ont été retirés ou restreints à la suite de signaux de pharmacovigilance, et des effets rares ont été identifiés et quantifiés pendant la campagne de 2021. Reconnaître qu'un vaccin a des effets indésirables n'est donc pas une concession faite aux opposants, c'est la description exacte du fonctionnement du système.

La balance bénéfice-risque compare les risques évités aux effets indésirables encourus. Elle n'est jamais universelle : elle dépend de l'âge, de l'état de santé et de la circulation de la maladie. C'est précisément pourquoi les recommandations diffèrent selon les populations, et pourquoi une recommandation peut changer quand l'épidémiologie change — ce qui est souvent lu comme une contradiction alors que c'est une mise à jour.`],
    ["Le doute et ses effets", `L'hésitation vaccinale désigne un retard ou un refus de vaccination malgré la disponibilité des vaccins. L'Organisation mondiale de la santé l'a classée parmi les principales menaces pour la santé mondiale. Ses ressorts ne se réduisent pas à la désinformation : défiance envers les autorités et l'industrie, mémoire d'affaires sanitaires antérieures, sentiment que la maladie a disparu — paradoxe cruel du succès vaccinal, qui rend invisible ce qu'il évite.

La France figure parmi les pays où cette défiance est la plus élevée, un contraste notable pour la patrie de Pasteur.

La poliomyélite illustre ce qui est en jeu. Presque éradiquée par la vaccination — les cas mondiaux sont passés de plusieurs centaines de milliers par an à quelques dizaines —, elle persiste dans quelques zones où les campagnes se heurtent aux conflits armés et aux rumeurs. Le dernier kilomètre d'une éradication est toujours le plus difficile, et le moindre relâchement peut faire repartir la transmission depuis un foyer résiduel.`],
  ],

  "sv3-cerveau-troubles": [
    ["Deux disciplines, deux regards", `La neurologie et la psychiatrie s'occupent du même organe mais relèvent d'approches et de spécialités distinctes. La première traite des atteintes dont on identifie une lésion ou un mécanisme physiopathologique — dégénérescence, inflammation, décharge électrique. La seconde traite des troubles définis par leurs manifestations psychiques et comportementales, sans lésion identifiable en imagerie de routine.

Cette frontière est historique autant que scientifique, et elle se déplace : chaque fois qu'un mécanisme est élucidé, un trouble passe de la psychiatrie à la neurologie — ce fut le cas de l'épilepsie et de la paralysie générale de la syphilis. Elle a des conséquences pratiques importantes sur l'organisation des soins, et alimente une stigmatisation persistante des troubles psychiatriques, jugés moins « réels » parce qu'invisibles à l'imagerie.`],
    ["Les maladies neurodégénératives", `La maladie d'Alzheimer est une maladie neurodégénérative provoquant une perte progressive de mémoire puis d'autonomie. Elle se caractérise par deux lésions : des plaques amyloïdes entre les neurones et une dégénérescence des protéines tau à l'intérieur. Le lien de causalité entre ces lésions et la maladie reste discuté, et l'échec de nombreux traitements anti-amyloïde a conduit à réexaminer l'hypothèse dominante.

La maladie de Parkinson résulte de la dégénérescence de neurones producteurs de dopamine dans une région précise du cerveau. Les symptômes n'apparaissent que lorsque la majorité de ces neurones ont disparu, ce qui laisse une longue phase silencieuse. Le traitement par précurseur de la dopamine améliore considérablement les symptômes sans ralentir la dégénérescence.

La sclérose en plaques est une maladie auto-immune où le système immunitaire détruit la gaine de myéline qui isole les fibres nerveuses, ce qui ralentit ou bloque la conduction. Elle évolue le plus souvent par poussées suivies de rémissions, et touche préférentiellement l'adulte jeune.

L'épilepsie, enfin, correspond à une décharge électrique anormale et synchronisée de populations de neurones. Ses manifestations dépendent entièrement de la région concernée, et vont de la crise convulsive généralisée à une simple absence de quelques secondes.`],
    ["Les troubles psychiatriques", `La dépression au sens clinique n'est pas une tristesse passagère : c'est un trouble caractérisé par une humeur basse durable, associée à une perte d'élan et de plaisir, à des troubles du sommeil, de l'appétit et de la concentration, pendant plusieurs semaines au moins. La confusion courante avec le découragement ordinaire nourrit l'injonction absurde de « se secouer », dont l'inefficacité est précisément un critère du trouble.

Un trouble bipolaire associe des épisodes dépressifs et des épisodes maniaques ou hypomaniaques — exaltation, réduction du besoin de sommeil, prises de risque, accélération de la pensée. Le diagnostic est souvent posé avec des années de retard, les phases dépressives amenant à consulter et les phases hautes n'étant pas rapportées comme un problème.

La schizophrénie est un trouble psychotique affectant la perception et la pensée : hallucinations, idées délirantes, désorganisation, mais aussi symptômes négatifs — retrait, appauvrissement de l'expression — souvent plus invalidants. Deux idées fausses persistent : elle n'a rien à voir avec un « dédoublement de la personnalité », et les personnes qui en souffrent sont bien plus souvent victimes qu'auteurs de violences.

Un trouble du spectre de l'autisme est un trouble du neurodéveloppement affectant la communication et les interactions sociales, avec des intérêts restreints et des particularités sensorielles. La notion de spectre est essentielle : les situations vont de l'autonomie complète à un besoin d'accompagnement permanent.`],
    ["Ce qui permet d'agir", `La neuroplasticité est la capacité du cerveau à réorganiser ses connexions, y compris à l'âge adulte. Elle fonde toute la rééducation après un accident vasculaire cérébral ou un traumatisme : des régions saines peuvent progressivement assurer des fonctions perdues, à condition d'un entraînement intensif et précoce. C'est aussi ce qui explique que la récupération dépende fortement de la rapidité et de l'intensité de la prise en charge.

La stimulation cérébrale profonde implante des électrodes dans des noyaux précis pour moduler leur activité par un courant permanent, réglable. Ses résultats sur les formes sévères de maladie de Parkinson sont spectaculaires — le tremblement peut cesser en quelques secondes à l'allumage —, et elle est explorée pour des troubles obsessionnels compulsifs et des dépressions résistantes. Elle ne guérit pas la maladie : elle en corrige les symptômes tant que le dispositif fonctionne.`],
  ],

  "sv3-hormones-reproduction": [
    ["Des messagers infimes", `Une hormone est une molécule sécrétée dans le sang par une glande, et qui agit à distance sur des cellules portant le récepteur correspondant. Sa particularité est la concentration : les hormones agissent à des doses extrêmement faibles, de l'ordre du nanogramme ou du picogramme par millilitre de sang — l'équivalent d'une pincée de sel dans une piscine.

Cette sensibilité extrême a une conséquence directe en toxicologie : elle explique pourquoi les perturbateurs endocriniens échappent au raisonnement classique. Une substance qui imite une hormone peut produire un effet à des doses considérées comme négligeables pour un toxique ordinaire, et les systèmes hormonaux étant régulés de façon non linéaire, une faible dose peut avoir un effet qu'une forte dose n'a pas. Les fenêtres d'exposition comptent en outre autant que les quantités : une exposition pendant la vie fœtale peut orienter durablement un développement.`],
    ["Grandir et mûrir", `L'hormone de croissance, sécrétée par l'hypophyse, commande la croissance pendant l'enfance, principalement en stimulant la production d'un facteur de croissance par le foie. Sa sécrétion est pulsatile et maximale pendant le sommeil profond — ce qui donne un fondement physiologique au conseil de coucher tôt les enfants.

La puberté n'est pas un démarrage mais une réactivation : l'axe hypothalamo-hypophysaire commandant les gonades fonctionne pendant la vie fœtale et les premiers mois, puis se met en veille pendant toute l'enfance avant de reprendre. Ce qui déclenche cette reprise reste incomplètement compris, mais elle dépend notamment de signaux liés à la masse grasse — ce qui explique le lien entre nutrition et âge de la puberté, et l'avancée de celle-ci observée depuis un siècle.

La thyroxine, sécrétée par la thyroïde, règle le métabolisme de base : elle fixe la vitesse à laquelle l'organisme consomme son énergie. Son excès accélère tout — rythme cardiaque, transit, amaigrissement, nervosité — et son défaut ralentit tout. C'est l'hormone dont le dosage est le plus prescrit, et son insuffisance congénitale fait partie du dépistage néonatal parce qu'elle compromet le développement cérébral.`],
    ["Deux diabètes qui n'ont en commun qu'un nom", `Le diabète de type 1 résulte d'une destruction auto-immune des cellules du pancréas qui produisent l'insuline. Il apparaît souvent dans l'enfance ou chez l'adulte jeune, brutalement, et impose un apport d'insuline à vie. Il n'a aucun rapport avec le mode de vie.

Le diabète de type 2 résulte d'une résistance progressive des cellules à l'insuline, longtemps compensée par une sécrétion accrue jusqu'à l'épuisement du pancréas. Il est fortement lié au surpoids, à la sédentarité et à une prédisposition génétique, s'installe silencieusement sur des années, et représente plus de neuf cas sur dix.

Les confondre conduit à des malentendus blessants — reprocher son mode de vie à une personne dont le pancréas a été détruit par son propre système immunitaire — et à des erreurs de compréhension des traitements.`],
    ["Le principe qui gouverne tout", `Le cortisol est l'hormone du stress, sécrétée par les glandes surrénales : elle mobilise l'énergie, augmente la glycémie et met l'organisme en état de réponse. Elle régule aussi le métabolisme au quotidien, avec un pic matinal qui participe au réveil. Son excès chronique dégrade la mémoire, le sommeil, l'immunité et le métabolisme — le stress prolongé n'est pas une plainte subjective mais un dérèglement mesurable.

L'hormone parathyroïdienne régule le calcium sanguin, en le mobilisant depuis l'os quand il vient à manquer. Cette priorité donnée au taux sanguin sur la solidité du squelette rappelle une règle générale de la physiologie : le milieu intérieur est défendu avant les réserves.

Le mécanisme commun à ces systèmes est le rétrocontrôle : l'hormone produite freine sa propre commande, ce qui stabilise la concentration comme un thermostat stabilise une température. C'est ce qui rend l'exploration hormonale délicate — un dosage isolé ne signifie rien. Il faut interpréter l'hormone périphérique et l'hormone de commande ensemble, tenir compte de l'heure du prélèvement pour celles qui suivent un rythme circadien, et parfois recourir à des tests dynamiques de stimulation ou de freinage. Un chiffre hors norme n'est pas un diagnostic.`],
  ],

  "sv3-eau-milieux": [
    ["Lire une rivière", `Le droit français classe les cours d'eau en deux catégories piscicoles. Un cours d'eau de première catégorie est celui où dominent la truite et les salmonidés : eaux fraîches, vives, bien oxygénées, généralement en amont. La seconde catégorie regroupe les eaux plus lentes et plus chaudes à cyprinidés. Ce classement n'est pas qu'administratif : il traduit un gradient écologique réel, de la source à l'estuaire, où la température, l'oxygène, la pente et le substrat changent ensemble.

La continuité écologique désigne la libre circulation des espèces et des sédiments le long du cours d'eau. Les barrages, seuils et moulins la rompent : ils bloquent la montaison des poissons, retiennent les sédiments qui manqueront en aval, réchauffent l'eau et transforment un milieu courant en milieu stagnant. Une passe à poissons permet le franchissement, par une succession de bassins ou une rampe à faible pente, mais son efficacité est très variable et jamais totale.

Un poisson migrateur amphihalin passe d'eau douce en eau salée au cours de sa vie : le saumon et l'alose remontent les fleuves pour se reproduire, l'anguille fait l'inverse en allant pondre dans la mer des Sargasses. Ces espèces sont les premières victimes de la fragmentation des cours d'eau, et l'anguille européenne a perdu plus de quatre-vingt-dix pour cent de son recrutement en quelques décennies.`],
    ["Les berges et les tourbières", `Une ripisylve est la végétation boisée des berges d'un cours d'eau — aulnes, saules, frênes. Son rôle dépasse largement le paysage : elle stabilise la berge par ses racines, ombrage l'eau et limite son réchauffement, filtre les ruissellements chargés en nitrates avant qu'ils n'atteignent la rivière, fournit aux poissons abris et nourriture par les insectes qui en tombent. Sa suppression pour gagner quelques mètres de culture coûte bien plus qu'elle ne rapporte.

Une tourbière est une zone humide où la matière organique s'accumule sans se décomposer, faute d'oxygène dans un sol gorgé d'eau. Elle croît de quelques millimètres par siècle, et certaines se sont formées depuis la dernière glaciation. Ce sont les plus formidables stocks de carbone au mètre carré de la planète — bien davantage qu'une forêt — et leur drainage les transforme en émetteurs massifs. Elles conservent en outre remarquablement les vestiges organiques, comme en témoignent les corps des tourbières danoises vieux de deux millénaires.`],
    ["Mesurer la santé d'un milieu", `Un indice biologique de qualité de l'eau évalue le milieu à partir des organismes qui y vivent — larves d'insectes, mollusques, diatomées, poissons. Sa supériorité sur l'analyse chimique tient à ce qu'il intègre le temps : une analyse d'eau donne l'état à l'instant du prélèvement et manque un rejet ponctuel survenu la veille, tandis qu'une communauté d'invertébrés reflète les conditions des mois écoulés.

Certains taxons sont très sensibles à la pollution et à la désoxygénation — larves de plécoptères, d'éphémères, de trichoptères —, d'autres très tolérants. La composition du peuplement donne donc une note, standardisée et comparable entre stations. C'est la méthode de référence de la directive-cadre européenne sur l'eau, qui a fait passer l'objectif d'une qualité chimique à un « bon état écologique ».

L'eutrophisation d'un lac est l'enrichissement excessif en nutriments, azote et phosphore, qui provoque des proliférations d'algues et de cyanobactéries, l'opacification de l'eau, puis la désoxygénation du fond lors de leur décomposition. Elle est très difficile à inverser car le phosphore accumulé dans les sédiments continue d'alimenter le phénomène longtemps après l'arrêt des apports.`],
    ["Ce qui arrive et ce qui disparaît", `Une espèce aquatique invasive est une espèce introduite qui déséquilibre les milieux. L'écrevisse de Louisiane en offre le cas le plus visible en France : introduite pour l'aquaculture, échappée, elle creuse les berges, consomme la végétation aquatique et les pontes d'amphibiens, trouble l'eau, et transmet aux écrevisses indigènes une maladie à laquelle elle-même résiste. Les trois espèces d'écrevisses autochtones sont aujourd'hui menacées.

La convention de Ramsar, signée en 1971 dans la ville iranienne du même nom, est le premier traité international consacré à un type de milieu : elle engage les États à inscrire et à gérer des zones humides d'importance internationale. Son existence même signale la gravité du problème — les zones humides ont perdu une part considérable de leur surface en un siècle, drainées pour l'agriculture, l'urbanisation ou par assèchement lié aux prélèvements.
